...
-- 장점? 연락처 저장 시 수첩(아날로그) vs 스마트폰(디지털)
...
-- 생활 속 DB들 -> 다양한 영역을 대체(예: 스마트폰 연락처, 유튜브 영상, 인스타그램 사진/댓글/좋아요, 이번 달 카드 사용 내역 등)
...
-- 기능? DBMS는 사용자에게 다양한 기능을 제공(도서관 사서가 도서 대여/반납을 도와주는 느낌)
-- 아래 7가지 기능 암기X
-- - 효율적 데이터 관리
--   - 대량의 데이터를 체계적 저장, 관리
...
-- 무엇? DB와의 소통을 위한 언어(한국에선 한국어, 미국에선 영어, DB에선 SQL)
-- - 표준화된 질의 언어
-- - DBMS를 통해 수행됨
...
-- 중요! MySQL 관리자(Root) 비밀번호 설정: mysql1234
